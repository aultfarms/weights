...
const pages = ['Activity', 'Ledger', 'Balance Sheet', 'Profit Loss', 'Chart', '1099', 'Inventory' ];
...
export const NavBar = observer(() => {
  ...
  const menuItemClickedNav = (page: string) => () => {
    ...
    switch(page) {
      ...
      case 'Chart': actions.page('chart'); break;
      ...
      <Container maxWidth="xl">
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
            ...
            >
              {pages.map((page) => (
                <MenuItem key={page} onClick={menuItemClickedNav(page)}>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
            {pages.map((page) => (
              <Button
                key={page}
                onClick={menuItemClickedNav(page)}
          ...
          <Box sx={{ flexGrow: 0 }}>
            ...
            >
              {settings.map((setting) => (
                <MenuItem key={setting} onClick={menuItemClickedUser(setting)}>
