...
consts = {
        expenses_listid: "520d13ae3c4a3e05420001d4", // Board in Ault Farms org.
...
var populateSum = function() {
  ...
  Trello.get("lists/"+consts.expenses_listid+"/cards", function(data) {
    var sum = 0;
    var html = "<table><tr><th>Amount</th><th>Total</th><th>Card</th></tr>";
    for (var i in data) {
      var regexp = /\$([0-9,.]+)/g;
      while (1) {
        ...
        html += "<tr><td class=\"amt\">"+famt+"</td><td class=\"sum\">"+fsum+"</td><td class=\"card\">"+data[i].name+"</td></tr>";
    ...
    $("#d_sum").html("Total dollars spent: " + formatted_sum + " on " + data.length + " cards.<br/>" + html);
