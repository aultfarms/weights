...
export function accountToWorkbook(acct: { 
  ...
  let row=0;
  const headerindex: { [key: string]: true } = {};
  ...
  const formatDate = (date: Moment | string | undefined | null) => {
  ...
  for (const tx of acct.lines) {
    for (const [col, key] of header.entries()) {
      ...
      switch (key) {
        ...
        break;
        case 'note': 
          if (typeof tx.note === 'object') v = objToStr(tx.note); 
          t = 's';
        break;
