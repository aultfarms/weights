...
export default async function run(client: Trello.client.Client, t: typeof Trello) {
  ...
  if (!genericlist2) throw new Error('ERROR: could not find GenericList2 in GenericTests board'); // for updateExistingCardNameAndMoveToBottomOfList
  ...
  if (!listafter2) throw new Error('Could not retrieve GenericList2 after updating card');
  const lastcard2 = listafter.cards?.slice(-1)?.[0];
  if (!lastcard2) throw new Error('There is no last card in GenericList2 after updateExistingCardNameAndMoveToBottomOfList');
  if (lastcard2.name !== name) throw new Error('Name of last card is not '+name);
