...
export default async function run(a, accounts) {
    ...
    if (!mkt)
        throw `annualBalanceSheet returned falsey for mkt`;
    if (!tax)
        throw `annualBalanceSheet returned falsey for tax`;
    // Should have yearend and 4 quarters
    if (!mkt.yearend)
        throw `Did not have yearend on mkt`;
    if (!tax.yearend)
        throw `Did not have yearend on tax`;
    if (mkt.quarters?.length !== 4)
        throw `Did not have 4 quarters on mkt`;
    if (tax.quarters?.length !== 4)
        throw `Did not have 4 quarters on tax`;
