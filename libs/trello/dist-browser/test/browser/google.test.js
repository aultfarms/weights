...
export default async function run(accounts) {
    ...
    if (!r1)
        throw `uploadXlsxWorkbookToGoogle returned null`;
    const r2 = await drive.getFileContents({ id: r1.id });
    if (!r2)
        throw `drive.getFileContents returned null`;
    const wb2 = xlsx.read(r2, { type: 'array' });
    const data2 = xlsx.utils.sheet_to_json(wb2.Sheets['testsheet']);
    if (!deepequal(data, data2))
        throw `Downloaded xlsx differs from uploaded xlsx.  Uploaded = ${stringify(data)}, Downloaded = ${stringify(data2)}`;
