...
//const silent = () => {};
export default async function run(a, rawaccts) {
    if (rawaccts.length < 1)
        throw `You did not pass any accoutns to test.`;
...
function basicVerify(r, orig, settings) {
    if (!r)
        throw `null result`;
    if (r.errors && r.errors.length > 0)
        throw `Account returned errors: ${r.errors.join('\n')}`;
    if (settings) {
        if (!deepequal(r.settings, settings))
            throw `Account settings (${JSON.stringify(r.settings)}) do not match expected (${JSON.stringify(settings)})`;
    }
    // Validate all the lines: should have lineno, should leave dates alone, numbers should be numbers
    if (!r?.lines)
        throw `Account has no lines`;
    for (const l of r.lines) {
        if (!l.lineno)
            throw `Line has no lineno`;
...
function testInitialValidateAccounts(a) {
    ...
    if (res.length !== 1)
        throw `Only sent in one account, but got bat ${res.length} accounts`;
    ...
    if (res?.[0]?.lines) {
        ...
        if (hasemptyline)
            throw `Found line in result with emptykey, it should have been removed as an empty line`;
        if (haslineno)
            throw `Found line in result with lineno of the emptykey line, it should have been removed`;
    }
    info('passed initialValidateAccounts eliminate empty lines');
}
function testAssetsToTxAccts(a) {
    ...
    if (!res || !res[0])
        throw `assetsToTxAccts returned null or no accounts`;
    if (res[0].errors && res[0].errors.length > 0)
        throw `Account had errors: ${res[0].errors.join('\n')}`;
    if (res.length !== 2)
        throw `assetsToTxAccts should have returned 2 accounts: 1 tax and 1 mkt, instead it returned ${res.length} with names ${res.map(a => a.name).join(', ')}`;
    ...
    if (res[0].errors && res[0].errors.length > 0)
        throw `Account had errors: ${res[0].errors.join('\n')}`;
    if (res.length !== 1)
        throw `assetsToTxAccts should have only returned 1 cash account, instead it returned ${res.length} with names ${res.map(a => a.name).join(', ')}`;
    if (!deepequal(accts, res))
        throw `assetsToTxAccts changed the cash account and it should have left it alone`;
    info('passed assetsToTxAccts leaving cash account alone');
}
function testStandardize(a) {
    ...
    if (!res[0])
        throw `No test account 0 (TS error)`;
    ...
    if (lines[0]?.description !== 'START' && !lines[0]?.isStart)
        throw `First line is not start`;
    if (lines[1]?.amount !== -100000)
        throw `Second line amount (${lines[1]?.amount}) not properly derived from debit`;
    if (!lines[3] ||
        !lines[3].note ||
...
function testSplits(a) {
    ...
    if (lines.length < 0)
        throw `No lines returned from splits`;
    if (lines.find(l => l.category === 'SPLIT'))
        throw `There is still a line in the result with a category of SPLIT`;
    // Lines 6 and 7 from original were split lines, they are now 5 and 6 (base line 5 should have been removed)
    if (lines.find(l => l.who === 'SPLIT'))
        throw `There is still a line with 'who' as a SPLIT`;
    ...
    let acct = res[0];
    if (!acct)
        throw `No account returned from splits on the intentional messed up splitAmount test`;
    if ((acct.errors?.length || 0) < 1)
        throw `Tested splitAmount was intentionally wrong, but there was not exactly 1 error in result (${acct.errors?.join('\n')})`;
    if (!acct.errors?.find(e => e.match(/LINE: 6/)))
        throw `Did not find error for line 6 in result`;
    if (!acct.errors?.find(e => e.match(/sum +of +splits/)))
        throw `Did not find 'sum of splits' error in result`;
    info('passed checking a messed up splitAmount');
}
function testAssertAllAccounts(a) {
    ...
    if (!failed)
        throw `assertAllAccounts failed to throw on invalid accounts`;
    info('passed assertion on un-processed accounts');
}
function testValidateBalanaces(a) {
...
async function testLoadInSteps(a, rawaccts) {
    info('It should load all the example spreadsheets without error');
    if (rawaccts.length < 0)
        throw `You have no RawSheetAccount's to test`;
    ...
    try {
        for await (step of steps) {
            ...
            if (!step)
                throw `Step ${count}: generator returned null`;
            count++;
        }
    }
    catch (e) {
    ...
    const assertFinalAccounts = a.assertFinalAccounts;
    ...
    if (accts.length !== 26)
        throw `There should be 26 accounts in final.tax, but there are ${accts.length} instead.  They are: ${accts.map(a => a.name).join('\n')}`;
    accts = step.final.mkt.accts;
    if (accts.length !== 31)
        throw `There should be 31 accounts in final.mkt, but there are ${accts.length} instead.  They are: ${accts.map(a => a.name).join('\n')}`;
    info('passed right number of final tax and mkt accounts');
    return step.final;
}
function testAccountToWorkbook(a, acct) {
