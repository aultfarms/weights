...
export function ten99({ ledger, year, settings }) {
    ...
    const all_settings_names = settings.people
        .reduce((acc, p) => ([...acc, p.name, ...p.othernames]), [])
        .filter(uniqueFilter);
    const required_categories = settings.categories.filter(c => c.alwaysReport);
    const missing_people_from_required_categories = {};
...
function categoryRegExp(searchname) {
