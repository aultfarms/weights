...
export function amount(cat, cfg) {
    cfg = cfg || {};
    const { start, end, timerange, type, only, exclude } = cfg;
    // if this one it explicitly excluded, no need to recurse
    if (exclude && cat.name === exclude)
        return 0;
    // if this one does not contain the only one we want, we're done
    if (only && !containsCategory(cat, only) && !underCategory(cat, only))
        return 0;
    // total of transactions at this level plus amounts of children
    const mysum = cat.transactions.reduce((sum, tx) => {
        if (type === 'credit' && tx.amount < 0)
            return sum; // credits are positive
        if (type === 'debit' && tx.amount > 0)
            return sum; // debits are negative
        if (start && tx.date.isBefore(start))
            return sum; // before start time
        if (end && tx.date.isAfter(end))
            return sum; // after end time
        if (timerange && !timerange.contains(tx.date))
            return sum; // does not fall within the time range
    ...
    if (Math.abs(ret) < 0.01)
        return 0; // If you don't do this, you get a NaN from numeral sometimes
    return ret;
}
export function debit(cat, cfg) {
    if (!cfg)
        cfg = {};
    return amount(cat, { ...cfg, type: 'debit' });
}
export function credit(cat, cfg) {
    if (!cfg)
        cfg = {};
    return amount(cat, { ...cfg, type: 'credit' });
}
export function treeToCategoryNames(tree, catindex, // mutates catindex
opts, parentstr) {
...
;
export function getCategory(cat, name) {
    if (!cat)
        return null;
    if (name === 'root')
        return cat;
...
export function containsCategory(cat, name) {
    if (!cat)
        return false;
    if (cat.name === name)
        return true;
    ...
    for (const child of Object.values(cat.children)) {
        if (containsCategory(child, name))
            return true;
    }
    return false; // none of children had it either
}
export function underCategory(cat, search_parent_name) {
    if (!cat.parent)
        return false; // reached the top
    if (cat.parent.name === search_parent_name)
        return true;
    return underCategory(cat.parent, search_parent_name);
}
;
export function categorize({ lines }) {
    const template = (name, parent) => {
        return {
            name,
            parent: parent || null,
            children: {},
    ...
    const transactionToTree = (catarr, tx, treelevel) => {
        ...
        if (!treelevel.children[catkey])
            treelevel.children[catkey] = template(catkey, treelevel);
