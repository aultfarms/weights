...
;
export function getAccountBalance({ balanceSheet, accountName, balanceTree }) {
    const parts = accountName.split('-');
    if (parts.length < 1)
        throw new MultiError({ msg: 'accountName was empty' });
...
*/
export function balanceForAccountOnDate(d, acct) {
    ...
    for (const l of acct.lines) {
        ...
        if (!isSameDayOrBefore(l.date, d)) { //l.date.isSameOrBefore(d)) {
            if (!prev)
                return 0; // first line in account that is not same/before search date
            return prev.balance;
        }
        prev = l;
    }
...
function updateBalancesFromChildren(tree, asOfDate) {
...
function balanceTreeFromAcctsOnDate(accts, asOfDate) {
    const tree = accts.reduce((acc, acct) => {
        ...
        for (const p of parts) {
            ...
            if (!cur.children)
                cur.children = {};
            if (!cur.children[p])
                cur.children[p] = { name: p, balance: 0, children: {} };
            cur = cur.children[p];
        }
...
export async function annualBalanceSheet({ year, date, type, ledger }) {
    ...
    if (year) {
        dates.push({ date: `${year}-12-31`, name: `${year}Q4`, isQuarter: true, isYearEnd: true }, { date: `${year}-09-30`, name: `${year}Q3`, isQuarter: true }, { date: `${year}-06-30`, name: `${year}Q2`, isQuarter: true }, { date: `${year}-03-31`, name: `${year}Q1`, isQuarter: true });
    }
