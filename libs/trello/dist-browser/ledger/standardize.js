...
function isBeforeDay(a, b) {
    ...
    if (astr === bstr)
        return false; // they are the same day, so "Not before"
    return a.isBefore(b);
}
    ...
    return accts.map(acct => {
        ...
        acct.lines = util.mapSkipErrors(acct.lines, line => {
            try {
                ...
                let splitAmount = numeral(line.splitAmount).value() || 0;
                // You can set amounttype as 'inverted' in a SETTINGS line
                if (acct.settings.amounttype === 'inverted') {
                    amount *= -1.0;
                    splitAmount *= -1.0;
                }
                const is_split = line.description?.trim() === 'SPLIT';
                ...
                // Balance
                let balance = numeral(line.balance).value() || 0;
...
function isFutures(line) {
    if (!exists(line.txtype))
        return false;
    if (!exists(line.commodity))
        return false;
    return true;
}
...
function parseAmountFromDebitCredit(line) {
    let debit = 0;
    let credit = 0;
    if (line.debit)
        debit = numeral(line.debit).value();
    if (line.credit)
        credit = numeral(line.credit).value();
...
function parseDateFromWrittenOrPost(line, is_debit) {
    ...
    let postDate = null;
    if (line.writtenDate)
        writtenDate = parseDate(line.writtenDate);
    if (!writtenDate || !isMoment(writtenDate))
        writtenDate = null;
    if (line.postDate)
        postDate = parseDate(line.postDate);
    if (!postDate || !isMoment(postDate))
        postDate = null;
