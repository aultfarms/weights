...
const AMOUNT_PLACEHOLDER_FOR_ASOF = -9999999999;
/*
...
function safesetNumber(n) {
    if (!n)
        return 0; // handles undefined, 0, ''
    if (typeof n === 'string')
        return numeral(n)?.value() || 0;
    return n;
}
function pushError(acct, err) {
    acct.errors = err.concat(acct.errors || []);
}
function balanceAtDate(date, acct) {
    ...
    for (const l of acct.lines) {
        ...
        if (l.date && isMoment(l.date) && l.date.isAfter(date))
            break; // found first one after our target, so balance is whatever we had last
        balance = l.balance || 0;
    }
    return balance;
}
...
function ensureOnlyOneOfAssetTxType(lines, candidate) {
...
function assetLinesToAccts(accumulator, acct) {
    ...
    for (const line of acct.lines) {
        ...
        if (acct.settings.acctname) {
            ...
            if (!line.category)
                line.category = name; // mainly for futures-asset
        }
...
export default function ({ accts }) {
    ...
    for (const acct of newaccts) {
        if (acct.errors && acct.errors.length > 0)
            continue; // if the account already has an error, ignore further processing
        switch (acct.settings.accounttype) {
            ...
            case 'futures-asset':
                ...
                if (acct.origin.lines) {
                    acct.origin.lines = acct.origin.lines.sort((a, b) => +(a.date) - +(b.date));
                }
                //------------------------------------------------------------------
                ...
                if (acct.settings.accounttype === 'futures-asset') {
                    ...
                    acct.origin.lines = unique_asofdates.map(d => {
                        const lines_w_date = acct.origin?.lines.filter(l => l.asOfDate === d) || [];
                        const cumulative_line = omit([
                ...
                const newlinesAndErrors = acct.origin.lines.reduce((acc, l) => {
                    try {
                            ...
                            if (weHave(value)) {
                                ...
                                if (l.priorDate) {
                                    if (initialLine.date.isBefore(l.priorDate)) {
                                        priorAsOfLine = {
                                            ...deepclone(linetemplate),
                                            date: moment(`${l.priorDate.format('YYYY-MM-DD')} 23:59:59`, 'YYYY-MM-DD HH:mm:ss'),
                        ...
                        if (l.asOfDate) { // all asset types, including futures-asset
                            ...
                            asOfLine = {
                                ...deepclone(linetemplate),
                                date: moment(`${l.asOfDate} 23:59:59`, 'YYYY-MM-DD HH:mm:ss'),
                        ...
                        if (initialLine) {
                            if (ensureOnlyOneOfAssetTxType(acc.lines, initialLine)) {
                                ...
                                if (priorAsOfLine)
                                    lines_to_push.push(priorAsOfLine);
                            }
                        }
                        if (saleLine) {
                ...
                newlinesAndErrors.lines.sort((a, b) => (+(a.date) - +(b.date)));
...
//# sourceMappingURL=assetsToTxAccts.js.map