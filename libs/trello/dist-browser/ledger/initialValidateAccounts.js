...
import { assertValidatedRawTx, assertAccountSettings, assertInventoryAccountSettings, assertLivestockInventoryAccountSettings } from './types.js';
...
// before standardize:
function applyLineNumber(l, index) {
    if (l.lineno) {
...
function fixCurrencyNumbers(l) {
    // Check/Fix every property of l
    for (const [k, v] of Object.entries(l)) {
        if (typeof v !== 'string')
            continue; // leave numbers alone
        if (k === 'description' || k === 'note')
            continue; // don't convert these to numbers
        let str = v.trim();
        // Is it a date?
...
function getAccountSettings({ acct, status }) {
    ...
    try {
        assertAccountSettings(settings);
    }
    catch (e) {
        status(red('FAILURE: acct ') + magenta(acct.name) + red(' has invalid Settings: ') +
            stringify(settings) +
...
function isCommentSettingsIgnoreOrEmptyLine(l) {
...
export default function ({ rawaccts, status = null }) {
    ...
    return rawaccts.map(acct => {
        ...
        let errors = [];
        ret.lines = acct.lines.reduce((acc, l, i) => {
        ...
        try {
            switch (ret.settings.accounttype) {
                ...
                case 'asset':
                    ...
                    assertColumns(ret, colnames, ['category', 'description', 'purchaseDate',
                ...
                case 'inventory':
                    try {
                        assertInventoryAccountSettings(ret.settings);
                    }
                    catch (e) {
                        throw AccountError.wrap(e, ret, 'accounttype was inventory, but settings failed validation for inventory account');
                    }
                    ...
                    if (ret.settings.inventorytype === 'livestock') {
                        try {
                            assertLivestockInventoryAccountSettings(ret.settings);
                        }
                        catch (e) {
                            throw AccountError.wrap(e, ret, 'accounttype was inventory, and inventorytype was livestock, but settings failed validation for livestock inventory account');
                        }
                        // Livestock inventory accounts also have these:
                        assertColumns(ret, colnames, ['weight', 'weightBalance', 'aveValuePerWeight', 'taxAmount', 'taxBalance']);
                    }
                    else {
...
//# sourceMappingURL=initialValidateAccounts.js.map