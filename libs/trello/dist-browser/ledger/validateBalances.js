        ...
        try {
            for (const [index, line] of acct.lines.entries()) {
                ...
                if (!moneyEquals(balance, line.balance))
                    throw new LineError({ line, msg: `Balance (${line.balance}) != computed balance from amounts (${balance})` });
                if (isInventory) {
                    if (!moneyEquals(qtyBalance, line.qtyBalance))
                        throw new LineError({ line, msg: `qtyBalance (${line.qtyBalance}) != computed balance from qty's (${qtyBalance})` });
                }
                if (isLivestock) {
                    if (!moneyEquals(weightBalance, line.weightBalance))
                        throw new LineError({ line, msg: `weightBalance (${line.weightBalance}) != computed balance from weight's (${weightBalance})` });
                    if (!moneyEquals(taxBalance, line.taxBalance))
                        throw new LineError({ line, msg: `taxBalance (${line.taxBalance}) != computed balance from taxAmount's (${taxBalance})` });
                }
            }
            goodaccts.push(acct);
        }
...
;
//# sourceMappingURL=validateBalances.js.map