...
import type { Tag, TagRange, Group } from '../types';
...
export const rangeContainsTag = (r: TagRange, tag: Tag): boolean => {
  return (
...
export const groupContainsTag = (group: Group,tag: Tag): boolean => {
...
export const groupForTag = (
  ...
  const asOfDate: Moment = asOfDateString ? moment(asOfDateString, 'YYYY-MM-DD') : moment();
...
export const tagStrToObj = (str:string): Tag => {
  str = str.trim();
...
export const tagObjToStr = (t: Tag): string => {
  if (!t) {
