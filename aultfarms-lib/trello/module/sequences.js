...
export const authorize = [({ trello }) => trello.authorize().catch(e => {
  throw errors.TrelloAuthorizeError(e);
}), set(state`trello.authorized`, true)];

export const deauthorize = [set(state`trello.authorized`, false), sequence('deauthorize->authorize', authorize)];
...
export const loadList = [({ props, trello }) => {
  ...
  initialized = trello.get('members/me/boards', { fields: 'name,id,closed' }).filter(b => b && !b.closed).then(result => {
    const board = _.find(result, b => b.name === props.boardName);
    if (!board) {
      console.log('Could not find board ' + props.boardName + ', creating it.');
      return trello.post('boards', { name: props.boardName }).then(result => {
        ret.board = board;
      });
    }
    ret.board = board;
    ...
    // then get the labels:
  }).then(() => trello.get(`boards/${props.board.id}/labels`, { fields: 'id,name,color' })).then(labels => {
    ret.labels = labels;
  })

  // then get the list info:
  .then(() => trello.get('boards/' + props.board.id + '/lists', { fields: 'name,id,closed' })).filter(l => l && !l.closed).then(result => {
    const list = _.find(result, l => l.name === props.listName);
    if (!list) {
      console.log('Could not find list ' + props.list + ', creating it.');
      return trello.post('boards/' + props.board.id + '/lists', { name: props.listName }).then(list => {
        ret.list = list;
      });
    }
    ret.list = list;
  }).then(() => ret).catch(error => {
    throw new TrelloGetError('Failed during list initialization: ' + error);
  });
...
({ props, trello }) => trello.get('lists/' + props.list.id + '/cards', { fields: 'name,id,closed,desc,dateLastActivity,labels' }).filter(c => c && !c.closed).then(result => ({ cards: result })).catch(error => {
  throw TrelloGetError('Failed to get cards for list ' + list + ': ' + error);
}),
...
// }
export const putCard = [({ trello, props }) => {
  let method = trello.post;
  let url = 'cards/';
  if (props.card.id) {
    // card already exists, put instead of post
    method = trello.put;
    url += props.card.id + '/';
  }
  return method(url, { name: props.card.name, idList: props.card.idList }).catch(err => {
    throw errors.TrelloSaveError(err);
  });
}];
