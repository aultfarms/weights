...
export const authorize = [({ google }) => google.authorize().catch(e => {
  throw new GoogleAuthorizeError(e);
}), set(state`google.authorized`, true)];

export const deauthorize = [set(state`google.authorized`, false), sequence('deauthorize->authorize', authorize)];
...
export const loadSheetRows = [sequence('loadSheet->authorize', authorize), ({ google, props }) => google.idFromPath(props.path), ({ google, props }) => google.getAllRows({ id: props.id, worksheetName: props.worksheetName }), // goes into props.values
